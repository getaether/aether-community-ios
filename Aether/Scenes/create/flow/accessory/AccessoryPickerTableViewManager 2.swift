...
class AccessoryPickerTableViewManager: NSObject, UITableViewDelegate, UITableViewDataSource, AccessoryPickerTableViewCellDelegate {
    ...
    func numberOfSections(in tableView: UITableView) -> Int { facade.getRooms()?.count ?? 0 }
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard let rooms = facade.getRooms() else { return 0 }
    ...
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        guard let rooms = self.facade.getRooms()
        else { return "" }
